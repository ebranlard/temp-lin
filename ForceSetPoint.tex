...
%       The output vector is stored for each target azimuthal position $\v{\psi}_0$. 
   ...
   If the rotational speed is none zero and if the generator is on, \textit{ServoDyn} applies an offset to the controller variables (based on $\epsilon_\Omega$, see \autoref{sec:changesservodyn}).
